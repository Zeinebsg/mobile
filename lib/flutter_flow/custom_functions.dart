...
List<SeancesRecord>? filterBySearch(
  List<SeancesRecord>? seances,
  String? searchText,
) {
  ...
  List<SeancesRecord> results = [];

  for (var seance in seances) {
    String filmName = seance.filmName?.toLowerCase() ?? '';

    if (filmName.contains(searchLower)) {
      results.add(seance);
    }
  }
