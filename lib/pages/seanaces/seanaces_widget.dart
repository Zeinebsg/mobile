...
import 'dart:convert';
import 'seanaces_model.dart';
export 'seanaces_model.dart';
import '../owner_dashboard/owner_dashboard_widget.dart';
import '../../widgets/news_widget.dart';

// ============================================================
// WIDGET MÉTÉO POUR UNE DATE SPÉCIFIQUE
// ============================================================
class WeatherForDate extends StatefulWidget {
  ...
  @override
  State<WeatherForDate> createState() => _WeatherForDateState();
}

class _WeatherForDateState extends State<WeatherForDate> {
  late Future<Map<String, dynamic>> _weatherFuture;

  @override
  void initState() {
    super.initState();
    _weatherFuture = _getWeatherForDate();
  }
      ...
      if (response.statusCode == 200) {
        ...
        if (weatherForDay != null) {
          ...
          if (temp <= 5) {
            ...
            messagePrecaution = '☀️ CANICULE : Hydratez-vous !';
            couleurValue = 0xFFFF9800;
            icone = '☀️';
          } else if (condition.contains('pluie')) {
            messagePrecaution = '☔ PLUIE : Prenez un parapluie !';
            couleurValue = 0xFF4FC3F7;
            icone = '☔';
          } else if (condition.contains('orage')) {
            messagePrecaution = '⚡ ORAGE : Restez en sécurité !';
            couleurValue = 0xFFF44336;
            icone = '⚡';
          } else if (windSpeed >= 15) {
            messagePrecaution = '💨 VENT FORT : Soyez prudent !';
            couleurValue = 0xFF9E9E9E;
            icone = '💨';
          } else if (humidity >= 80) {
...
class SeanacesWidget extends StatefulWidget {
  const SeanacesWidget({super.key, this.selectedSeanceId});

  final String? selectedSeanceId;

  static String routeName = 'seanaces';
  static String routePath = '/seanaces';

  @override
  State<SeanacesWidget> createState() => _SeanacesWidgetState();
}
...
class _SeanacesWidgetState extends State<SeanacesWidget> {
  ...
  void _checkUserRole() {
    ...
    if (user != null) {
      setState(() {
        _isOwner = user.role == 'owner' || user.role == '"owner"';
      });
    }
  }
  ...
  bool _matchesDateFilter(DateTime? seanceDate) {
    ...
    switch (_dateFilter) {
      case 'aujourdhui':
        ...
        return seanceDay == todayDay;
      case 'demain':
        final tomorrowDay = DateTime(today.year, today.month, today.day + 1);
        return seanceDay == tomorrowDay;
      default:
        if (_selectedDate != null) {
          return seanceDay == _selectedDate;
        }
        return true;
    }
  }
  ...
  String _formatTime(DateTime? dt) {
    if (dt == null) return 'Heure inconnue';
...
''';
  ...
  Future<void> _editSeance(String docId) async {
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  Future<void> _deleteSeance(String docId) async {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            ...
            backgroundColor: Color(0xFF0A7407),
          ),
        );
      }
    }
  ...
  void _showSeanceModal(SeanceData seance) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
        ...
        children: [
          Container(
            ...
            child: ListView(
              ...
              children: [
                _buildDateFilterChip('Tous', 'tous'),
                const SizedBox(width: 8),
                _buildDateFilterChip('Aujourd\'hui', 'aujourdhui'),
                const SizedBox(width: 8),
                _buildDateFilterChip('Demain', 'demain'),
                const SizedBox(width: 8),
              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    var doc = filteredSeances[index];
                    var data = doc.data() as Map<String, dynamic>;
                    return _buildSeanceCard(doc.id, data);
                  },
                );
              },
            ),
  ...
  Widget _buildSeanceCard(String docId, Map<String, dynamic> data) {
    ...
    DateTime? dateTime = (data['dateTime'] as Timestamp?)?.toDate();
    String filmName = data['filmName'] as String? ?? 'Film sans titre';
  ...
  Widget _buildDateFilterChip(String label, String filterType, {DateTime? specificDate}) {
    ...
    } else if (filterType == 'aujourdhui') {
      isSelected = _dateFilter == 'aujourdhui';
    } else if (filterType == 'demain') {
      isSelected = _dateFilter == 'demain';
      ...
      onTap: () async {
        if (filterType == 'calendar') {
          ...
          setState(() {
            ...
            if (filterType == 'specific' && specificDate != null) {
